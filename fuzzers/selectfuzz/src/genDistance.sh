# Modified version of SelectFuzz: https://github.com/cuhk-seclab/SelectFuzz/blob/master/scripts/genDistance.sh

#!/bin/bash
if [ $# -lt 2 ]; then
  echo "Usage: $0 <binaries-directory> <temporary-directory>"
  echo ""
  exit 1
fi
...
if [ $RESUME -le $STEP ]; then
  ...
  for binary in $(echo "$binaries"); do
    ...
    opt -load $FUZZER/repo/libDFUZZPASS.so -DFUZZPASS $binary.0.0.preopt.bc -targets=$TMPDIR/BBtargets.txt -outdir=$TMPDIR
  ...
  $AFLGO/scripts/merge_callgraphs.py -o callgraph.dot $(ls callgraph.*)
