...
from benchmark import generate_slicing_worklist, SLICE_TARGETS, SLICE_LOC
...
SPARROW_PATH = os.path.join(FUZZER + '/sparrow', 'bin', 'sparrow')
...
def run_sparrow(works, thin, bug):
    PROCS=[]
    for prog in works:
        ...
        if bug == "all":
            for bug in bugs:
                slice_loc = SLICE_LOC[bug]
                cmd += ["-slice", bug + "=" + slice_loc]
        elif bug not in bugs:
            print("%s is not a candidate", bug)
            exit(1)
        else:
            slice_loc = SLICE_LOC[bug]
            cmd += ["-slice", bug + "=" + slice_loc]
...
def main():
    ...
    if len(sys.argv) != 4:
        print("Usage: %s <benchmark> <thin/naive> <bug>" % sys.argv[0])
        exit(1)
    ...
    while len(worklist) > 0:
        ...
        run_sparrow(works, thin, sys.argv[3])
