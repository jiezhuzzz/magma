...
# Copied from dafl customized llvm setup
...
# Pre-install for Sparrow. Last two however, do not appear in the original repo
apt-get update
apt-get install -y opam libclang-cpp12-dev libgmp-dev libclang-12-dev llvm-12-dev libmpfr-dev pkg-config zlib1g-dev

mkdir /benchmark
chown magma:magma /benchmark